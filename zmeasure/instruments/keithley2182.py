...
class Keithley2182(Driver):

    def __init__(self, gpib_addr=22,name='kl2182',readcode=[]):
        """
    ...
    def source_type(self):
        ...
        response = self.instrument.query("source:function:mode?").strip()
    ...
    def source_type(self, value):
        if value.lower() == 'voltage' or value.lower() == 'v':
            source = 'voltage'
            self.instrument.write("source:function:mode {}".format(source.lower()))
    ...
    def source_mode(self):
        ...
        # TODO: test
        return self.instrument.query('source:' + self.source_type.lower() + ':mode?')

    @source_mode.setter
    def source_mode(self, mode):
        if mode.lower() in ('fixed', 'sweep', 'list'):
            self.instrument.write('source:' + self.source_type.lower() + ':mode {}'.format(mode))
        else:
    ...
    def source_value(self):
        ...
        # TODO: test
        return self.instrument.query('source:' + self.source_type.lower() + ':level?')

    @source_value.setter
    def source_value(self, value):
        self.instrument.write("source:function:mode " + self.source_type.lower())
        self.instrument.write("source:" + self.source_type.lower() + ":range " + str(value))
        self.instrument.write("source:" + self.source_type.lower() + ":level " + str(value))
    ...
    def measure_type(self):
        ...
        measure_type_response = self.instrument.query("sense:function?").strip().replace('\"', '').split(',')[-1]
    ...
    def measure_type(self, value):
        ...
        if value.lower() in measure_type:
            self.instrument.write("sense:function:ON {}".format(measure_type[value.lower()]))
        else:
    ...
    def fetch(self, *measurements):
        ...
        """
        response = self.instrument.query('FETC?').strip().split(',')
    ...
    def partial_fetch(self):
        return self.fetch(*self.read_code)
    # Trigger functions
